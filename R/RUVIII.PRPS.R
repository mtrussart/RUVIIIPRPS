...
#' @param prps.name Symbol. sample names/ids or a name of column in sample annotation.
...
RUVIII.PRPS <- function(
    ...
    if(!is.null(prps.name)){
        if(prps.group == 'supervised'){
            ...
            if(sum(prps.name %in% names(se.obj@metadata$PRPS$supervised)) != length(prps.name)){
                stop('All or some of "prps.name" cannot be found in the SummarizedExperiment object.')
            }
            ...
            if(sum(prps.name %in% names(se.obj@metadata$PRPS$un.supervised)) != length(prps.name)){
                stop('All or some of "prps.name" cannot be found in the SummarizedExperiment object.')
            }
            ...
            if(sum(prps.name %in% all.sets != length(prps.name))){
                stop('All or some of "prps.name" cannot be found in the SummarizedExperiment object.')
            }
        } else if (prps.group == 'none'){
            stop('The "prps.group" cannot be "none" when the "prps.name" is not "NULL".')
        }
    }
    ...
    if(prps.group == 'supervised'){
        if(is.null(prps.name)){
            prps.name <- names(se.obj@metadata$PRPS$supervised)
        }
        printColoredMessage(
            message = paste0('- ', length(prps.name), ' supervised PRPS group(s) are found in the SummarizedExperiment object.'),
            color = 'blue',
            verbose = verbose)
        prps.data <- lapply(
            prps.name,
            function(x) {
                printColoredMessage(
                    message = paste0('- The prps group "', x, '" has ', length(se.obj@metadata$PRPS$supervised[[x]]$prps.data),
                                     ' PRPS sub_group(s).'),
                    color = 'blue',
                    verbose = verbose)
                se.obj@metadata$PRPS$supervised[[x]]$prps.data
        })
        names(prps.data) <- prps.name

    }

    ## un supervised
    if (prps.group == 'un.supervised'){
        if(is.null(prps.name)){
    ...
    printColoredMessage(
        ...
        verbose = verbose)
    printColoredMessage(
        message = paste0('* ', length(findRepeatingPatterns(vec = colnames(prps.data), n.repeat = 2)),
                         ' pseudo-samples of pseudo replicate'),
        color = 'blue',
        verbose = verbose)
    ...
    if (length(k) > 1) {
        ...
        sum(rowSums(M) != 1)
        sum(colSums(M) == 1)
        printColoredMessage(message = '- Obtain residuals from the control assays:',
                            color = 'blue',
                            verbose = verbose)
        Y0 <- fastResidop2(Y, M)
